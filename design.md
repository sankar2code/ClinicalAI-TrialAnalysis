...
### Semantic
- **Error** (`{colors.error}` — #c13515): Malformed/incomplete NCT ID in `analyze-bar-pill`/`nav-search`, "trial not found," failed analysis retry state. Deliberately distinct from Rausch. Reused as `{colors.confidence-low}` — see Confidence Ramp below.
- **Error Hover** (`{colors.error-hover}` — #b32505): Error-state link hover (e.g., "try again").
- **Success** (`{colors.success}` — #15803d): Real-time NCT ID validation — `analyze-bar-pill`/`nav-search`'s border and `{component.valid-id-check}`'s badge fill once the typed value fully matches NCT + 8 digits. Same green hue as `live-trial-stats`' `stat-completed-accent`. Reused as `{colors.confidence-very-high}` — see Confidence Ramp below. Trial status and epistemic-tag certainty still stay strictly ink/muted; only the confidence meter's numeric level is colored.
...
### Confidence Ramp
*Deliberate, explicit reversal of this app's original certainty-stays-grayscale rule — see `{component.confidence-meter}` and the Overview note above for why. Applies only to that one component; epistemic tags and trial status are unaffected.*
- **Confidence Low** (`{colors.confidence-low}` — #c13515, alias of `{colors.error}`): Reused rather than a new hue — "low confidence" already functions as a caution signal.
- **Confidence Medium** (`{colors.confidence-medium}` — #c2650a): New — the ramp's first intermediate stop, between error-red and confidence-high's yellow-green.
- **Confidence High** (`{colors.confidence-high}` — #65a30d): New — the ramp's second intermediate stop, between confidence-medium's orange and success-green.
- **Confidence Very High** (`{colors.confidence-very-high}` — #15803d, alias of `{colors.success}`): Reused rather than a new hue — "very high confidence" already functions as a trust signal.
...
**`analyze-bar-pill`** — White fill, `{rounded.full}`, 64px height, 1px hairline border, the system's one shadow tier at rest. A single segment holds the "NCT ID" caption label above a placeholder ("e.g. NCT04368728"). Friction-less: no submit button, no magnifying-glass icon — `lib/hooks/useNctIdAutoSubmit.ts` classifies the value on every keystroke, and the instant it fully matches `NCT` + 8 digits, the bar auto-navigates to the result after a 900ms pause (long enough for `{component.valid-id-check}` to visibly settle into place first, so the sequence reads as "check appears, *then* the page changes" rather than an instant jump). Four border states: `{colors.hairline}` while empty, `{colors.pending}` while the value is a valid, in-progress *prefix* of the pattern (so typing "NCT042…" reads as "still going," not wrong), `{colors.error}` the moment the value can no longer become valid (wrong prefix, too long, a non-digit after "NCT"), and `{colors.success}` plus `{component.valid-id-check}` at the trailing edge once it's fully valid. Text beneath the bar has two modes tied to the same states: a `{colors.muted}` format hint ("Format: 3 letters followed by 8 digits (e.g. NCT12345678)") while empty or typing — so the format is known before anyone has to get it wrong first — swapping to a `{colors.error}` "Enter the NCT ID in correct format/length" the moment it's actually invalid. Focus gets a `{colors.primary}` ring (`focus-within`) — Rausch's one remaining job on this bar, now that there's no button for it to color — but only while the value is empty. Once there's a value, the border color is already the state signal (`{colors.pending}`/`{colors.error}`/`{colors.success}`); a Rausch ring sitting right next to a differently-colored border on the same rounded edge read as two clashing colors rather than reinforcing anything, so the ring drops out the moment typing starts. Same conditional ring on `{component.nav-search}`.
...
**`confidence-meter`** — A compact horizontal 4-segment fill bar (Low / Medium / High / Very High) with a `{typography.body-sm}` caption beneath, both colored per level: `{colors.confidence-low}` → `{colors.confidence-medium}` → `{colors.confidence-high}` → `{colors.confidence-very-high}`, a red→orange→yellow-green→green ramp (`app/globals.css`). This is a deliberate, explicit reversal of this component's original design — it shipped ink-only for most of this app's life, specifically because design.md's Overview states certainty should never read as an automatic "danger"/"good" signal the way colored status does. Changed on direct request; the reasoning against it was raised and acknowledged before making the change, not overlooked. Low and Very High reuse `{colors.error}`/`{colors.success}` rather than inventing new hues at the ramp's endpoints — a plausible reuse, since "low confidence" and "very high confidence" already function as caution/trust signals respectively, which is exactly what error/success mean elsewhere in this app. The repeatable, restrained substitute for the source system's one-time 64px rating-display — this app needs a trust signal on every card, not one hero number per page. Always right-aligned to `{component.hypothesis-card}`'s header edge (`ml-auto`), its own two rows (bar + caption) right-aligned to each other too (`items-end`) — so once a long hypothesis title wraps and pushes the meter onto its own line, it stays pinned to the right rather than dropping to the header's left edge under the title.
...
**`upstream-error-illustration`** — The one deliberate illustration subject in an otherwise photo-free, icon-only app (a second, narrowly scoped exception to the "no photography/illustration anywhere" rule stated in `{component.hero-preview-card}`, granted specifically because a friendlier failure moment was worth the exception). A lab mouse — this app's own subject matter, trials run on lab studies — with one eyebrow raised, puzzling over a thought-bubble of a crossed-out signal glyph, rather than a generic broken-robot or sad-cloud cliché. Monochrome ink/muted/canvas throughout; the only color is a single stroke of `{colors.error}` slashing through the signal icon, reusing the same red already carrying every other failure state in this app rather than introducing a new one.

**`not-found-state`** — Renders for a well-formed but nonexistent NCT ID (engineering-doc.md Flow 4.3), and reused with an overridden message for the invalid-format case reached via a hand-typed URL (Flow 4.2). Pairs `{component.not-found-illustration}` with the same lab mouse established by `{component.upstream-error-illustration}`, and a body line that stays factually precise about *why* — correctly formatted, no matching study — while allowing itself one light aside ("even our lab mouse came up empty-pawed") rather than a flat, humorless restatement. `{colors.ink}` headline, not `{colors.error}` — a nonexistent ID isn't a system failure the way an upstream outage is, so it doesn't borrow that color.

**`not-found-illustration`** — Same mascot and construction as `{component.upstream-error-illustration}` (same body/head/ears/coat, same monochrome ink/muted/canvas palette), different pose for a different story: shrugging with both paws out beside a blank, dashed-border record card, rather than puzzling at a dead signal. "Searched, found nothing" instead of "couldn't reach the server" — the two illustrations are a small, consistent vocabulary, not one-off graphics each time.
...
| `{colors.confidence-medium}` | #c2650a | #fb923c |
| `{colors.confidence-high}` | #65a30d | #a3e635 |
